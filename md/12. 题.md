...
[闭包—从编译原理的角度理解作用域](闭包—从编译原理的角度理解作用域)
...
1. 如果缓存最大只允许寸3个组件，ABC三个组件一次进入缓存，没有问题。
...
当数据发生变化时，`setter` 方法调用 `Dep.notify()` 方法通知 `watcher` ，`watcher` 就会调用 `patch` 给真实的 `DOM` 打补丁，更新响应的视图。
